...
async def commit_scan(
    ...
    if image and image.filename:
        ...
        if len(raw) >= 64:
            try:
                _shrunk, _orig_kb, small_kb = _prepare_uploaded_image(raw)
                actual_kb = small_kb
...
class SettingsBody(BaseModel):
    ...
    monthly_wrvu_goal: float | None = None
...
def staff_stats(
    ...
    return {
        ...
        "daily_pace_wrvu": monthly_pace.daily_pace_wrvu,
        "expected_to_date_wrvu": monthly_pace.expected_to_date_wrvu,
        "pace_progress_percent": monthly_pace.pace_progress_percent,
        "is_on_pace": monthly_pace.is_on_pace,
...
async def staff_upload_op_note(
    ...
    """Snap an operative note; vision model transcribes text and stores text only by default."""
    surgeon, _ = auth
    wall_start_s = time.monotonic()
