...
- **Constraints:** [List]
- **UI Surface:** [Mobile PWA / Staff Portal / Admin / Trading Terminal / None]
- **Active Palette:** [Clinical Trust / Teal & Heal / Midnight Precise — see PALETTES.md]
```
...
## Don Naile Project Additions

### UI Assemble Checklist (any UI surface)

Before writing any component:
```
[ ] Read .cursor/rules/PALETTES.md — identify active palette for this project
[ ] Read memory.md (project root) — check for locked design decisions
[ ] Read .cursor/rules/CLAUDE.md → Design System section
[ ] Confirm mobile vs portal surface — different rules apply
[ ] Mobile: glass + waves + gradients (never flat)
[ ] Portal: sidebar + KPI strip + filter chips + dense tables
[ ] Never hardcode hex values — always use tokens from PALETTES.md
```

**Cal project:** Full app reference (routes, models, rules engine, run, templates) is in `docs/APP_REFERENCE.md`. Use it before changing code.

### HIPAA Stress-Test Checklist (SSS only)

Add to S phase for any SSS feature touching patient data:
```
[ ] No PHI in plaintext anywhere (DB, logs, URLs, error messages)
[ ] AES-256-GCM on all PHI fields
[ ] No PHI in localStorage, sessionStorage, or AsyncStorage
[ ] Audit log entry written for every PHI read/write
[ ] destroy_referral_phi() called on completion
[ ] HTTPS enforced — no HTTP fallback
[ ] Session token in memory only — never localStorage
```

### Deploy (Don-initiated only — never auto-deploy)

**SSS — llm-core (192.168.20.10):**
```bash
cd /home/dnaile748/sss/snapsendseen

# Single service (preferred — don't rebuild what didn't change)
docker compose -p snapsendseen_prod --env-file .env.prod -f docker-compose.prod.yml build --no-cache [service]
docker compose -p snapsendseen_prod --env-file .env.prod -f docker-compose.prod.yml up -d [service]

# Services: api | portal | patient | nginx

# Verify build has new code
docker exec sss_[service]_prod grep -r "[search_term]" /usr/share/nginx/html/assets/

# Logs
docker compose -p snapsendseen_prod -f docker-compose.prod.yml logs -f [service]
```

**Cal — llm-core (192.168.20.10):**
```bash
cd /home/dnaile748/cal
docker compose up -d --build cal_api
docker logs cal_api -f
```

**Northstar — northstar (192.168.1.116) — separate server:**
```bash
cd /opt/optionsbot
...
Update `memory.md` (in project root, e.g. `/home/dnaile748/cal/memory.md`) before closing:
```
[ ] What was completed this session
[ ] What is broken or in-progress
[ ] What to start with next session
[ ] Any new locked decisions to record
